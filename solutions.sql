...
-- QUERY 14 In the card table, how many cards exist for each type? Rank the result starting with the most frequent type.SECT
SELECT type, COUNT(card_id) as card_count FROM card
	GROUP BY type
...
SELECT date, COUNT(loan_id) as total_loan FROM loan
	WHERE date < 930907
    GROUP BY date
    ORDER BY date DESC;
